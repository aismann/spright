...
- __sprite rect__: the rectangular space a _sprite_ allocates on the _slice_.
- __sprite size__: the size of the _sprite rect_.
- __sprite bounds__: the logical bounds of a sprite - defined by the _sprite rect_ plus a margin.
...
| margin | sprite | [x], [y],<br/>[r-x, b-y] | Sets the margin around the sprite's rectangle, resulting in the logical bounds of the sprite. |
| crop | sprite | [boolean] | Adjust _margin_ to reduce the bounds to be relative to the the _trimmed rect_. |
| crop-pivot | sprite | [boolean] | Sets whether the sprite's pivot point should be relative to the _trimmed rect_. |
...
| min-size | sprite | x, [y] | Sets a sprite's minimum size, which is the space it allocates on the sheet. Smaller sprites are surrounded with transparency and aligned as defined by _align_. |
| divisible-size | sprite | x, [y] | Extends the sprite's size to be divisible by a certain number of pixels. |
| common-size | sprite | [key] | Makes all sprites with identical _keys_ expand to a common size. |
| align | sprite | align-x, align-y | Controls the placement of the source rect when increasing a sprite's size. The parametrization works as for _pivot_ points. |
| align-pivot | sprite | [key] | Adjust the rectangles of all sprites with identical _keys_, so their pivot points have identical offsets. |
...
| CSS                                               | [css.inja](templates/css.inja)         |
